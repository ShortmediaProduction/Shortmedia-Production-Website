export const mockData = {
  brand: {
    name: "Shortmedia",
    tagline: "Cinematic Storytelling & Professional Video Production"
  },
  
  about: {
    description: "Passionate videographer specializing in creating compelling visual stories that connect with audiences. From corporate imagefilms to social media content, I bring your vision to life through the art of cinematography."
  },

  services: [
    {
      title: "Imagefilme",
      description: "Professional corporate films that showcase your company's values, culture, and vision. Perfect for building brand awareness and connecting with your target audience through powerful visual storytelling.",
      icon: "camera"
    },
    {
      title: "Werbefilme", 
      description: "Creative advertising films that drive engagement and conversions. From concept to final cut, I create compelling commercials that capture attention and deliver your message effectively.",
      icon: "film"
    },
    {
      title: "Social Media Content",
      description: "Dynamic content optimized for social platforms. Short-form videos, stories, and reels that boost your online presence and engage your community across all social media channels.",
      icon: "smartphone"
    }
  ],

  achievements: [
    {
      number: "50+",
      label: "Projects Completed",
      description: "Successful video productions",
      icon: "projects"
    },
    {
      number: "25+",
      label: "Happy Clients",
      description: "Satisfied customers worldwide",
      icon: "clients"
    },
    {
      number: "2M+",
      label: "Total Views",
      description: "Across all platforms",
      icon: "views"
    },
    {
      number: "5",
      label: "Awards Won",
      description: "Industry recognitions",
      icon: "awards"
    }
  ],

  testimonials: [
    {
      name: "Sarah Mueller",
      position: "Marketing Director",
      company: "Tech Solutions GmbH",
      quote: "Shortmedia transformed our company story into a captivating visual narrative. The professionalism and creative vision exceeded our expectations.",
      rating: 5
    },
    {
      name: "Michael Schmidt",
      position: "CEO",
      company: "Innovation Labs",
      quote: "Outstanding work! The imagefilm perfectly captured our brand essence and significantly boosted our online engagement.",
      rating: 5
    },
    {
      name: "Lisa Weber",
      position: "Brand Manager",
      company: "Creative Agency",
      quote: "Working with Shortmedia was seamless. They delivered high-quality content that resonated perfectly with our target audience.",
      rating: 5
    }
  ],

  timeline: [
    {
      title: "First Camera",
      year: "2013",
      description: "Started my journey into visual storytelling with my first camera. This marked the beginning of my passion for capturing moments and telling stories through the lens.",
      icon: "camera"
    },
    {
      title: "Youtube Channel",
      year: "2018", 
      description: "Launched my YouTube channel to share creative content and connect with fellow video enthusiasts. This platform became my testing ground for new techniques and storytelling approaches.",
      icon: "youtube"
    },
    {
      title: "Mediamatik EFZ",
      year: "2021 - 2025",
      description: "Completed my professional education in Media Technology, gaining comprehensive knowledge in digital media production, post-production workflows, and technical expertise.",
      icon: "graduation"
    },
    {
      title: "Videographer at IWF AG",
      year: "2025",
      description: "Joined IWF AG as a professional videographer, working on diverse corporate projects and expanding my expertise in commercial video production.",
      icon: "briefcase"
    },
    {
      title: "Shortmedia",
      year: "2024 - 2026",
      description: "Founded Shortmedia to offer premium video production services. Focusing on creating cinematic experiences that elevate brands and tell meaningful stories.",
      icon: "star"
    }
  ],

  portfolio: {
    projects: [
      {
        id: 1,
        title: "Corporate Image Film",
        client: "Tech Startup XYZ",
        description: "A compelling story showcasing innovation and company culture through cinematic visuals and authentic testimonials that increased brand awareness by 150%.",
        thumbnail: "/api/placeholder/400/250",
        videoUrl: "https://www.youtube.com/embed/dQw4w9WgXcQ",
        tags: ["Imagefilm", "Corporate", "Storytelling"],
        duration: "3:45",
        year: "2024",
        views: "15.2K"
      },
      {
        id: 2,
        title: "Product Launch Campaign",
        client: "Design Agency ABC",
        description: "High-energy promotional content that generated 2M+ views and significantly boosted product awareness across multiple platforms.",
        thumbnail: "/api/placeholder/400/250", 
        videoUrl: "https://www.youtube.com/embed/dQw4w9WgXcQ",
        tags: ["Werbefilm", "Product", "Campaign"],
        duration: "2:15",
        year: "2024",
        views: "2.1M"
      },
      {
        id: 3,
        title: "Social Media Series",
        client: "Fashion Brand DEF",
        description: "A series of engaging short-form content that increased social media engagement by 300% and drove significant sales growth.",
        thumbnail: "/api/placeholder/400/250",
        videoUrl: "https://www.youtube.com/embed/dQw4w9WgXcQ", 
        tags: ["Social Media", "Fashion", "Series"],
        duration: "1:30",
        year: "2023",
        views: "500K"
      }
    ]
  }
};